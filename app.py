...
@app.route('/api/game/memoria')
def api_memoria():
    import random
    random_page = random.randint(1, 15) # Sorteia entre as 15 primeiras páginas de populares
    params = {"api_key": API_KEY, "language": LANGUAGE, "sort_by": "vote_count.desc", "page": random_page}
    try:
        res = requests.get(f"{BASE_URL}/discover/movie", params=params)
        res.raise_for_status()
        filmes = res.json().get('results', [])
        random.shuffle(filmes)
        selecionados = filmes[:12] # 12 filmes = 24 cards
        
        cards = []
        for f in selecionados:
            cards.append({"id": f['id'], "conteudo": f"https://image.tmdb.org/t/p/w500{f['poster_path']}", "tipo": "imagem"})
            cards.append({"id": f['id'], "conteudo": f['title'], "tipo": "texto"})
